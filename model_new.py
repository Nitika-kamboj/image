...
def fruit_input(if_random = True, if_training = True):
    if(if_training):
        filenames = [os.path.join("train-0000%d-of-00002.tfrecord" % i) for i in range(0, 1)]
    else:
        filenames = [os.path.join("validation-0000%d-of-00002.tfrecord" % i) for i in range(0, 1)]
...
def fruit_inference(image_batch):
    W_conv1 = weight_variable([5, 5, 3, 32])
    b_conv1 = bias_variable([32])

    x_image = tf.reshape(image_batch, [-1, IMAGE_SIZE, IMAGE_SIZE, 3])

    h_conv1 = new_func(conv2d(x_image, W_conv1) + b_conv1)
    h_pool1 = max_pool_2x2(h_conv1) # 112

    W_conv2 = weight_variable([5, 5, 32, 64])
    b_conv2 = bias_variable([64])

    h_conv2 = new_func(conv2d(h_pool1, W_conv2) + b_conv2)
    h_pool2 = max_pool_2x2(h_conv2) # 56

    W_conv3 = weight_variable([5, 5, 64, 128])
    b_conv3 = bias_variable([128])

    h_conv3 = new_func(conv2d(h_pool2, W_conv3) + b_conv3)
    h_pool3 = max_pool_2x2(h_conv3) # 28

    W_conv4 = weight_variable([5, 5, 128, 256])
    b_conv4 = bias_variable([256])

    h_conv4 = new_func(conv2d(h_pool3, W_conv4) + b_conv4)
    h_pool4 = max_pool_2x2(h_conv4) # 14

    W_conv5 = weight_variable([5, 5, 256, 256])
    b_conv5 = bias_variable([256])

    h_conv5 = new_func(conv2d(h_pool4, W_conv5) + b_conv5)
    ...
    h_fc1 = new_func(tf.matmul(h_pool5_flat, W_fc1) + b_fc1)
    ...
    h_fc2 = new_func(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)

    W_fc3 = weight_variable([256, 64])
    b_fc3 = bias_variable([64])

    h_fc3 = new_func(tf.matmul(h_fc2, W_fc3) + b_fc3)
...
def fruit_train(optimizer,learning_rate):
    start=time.time()
    print(optimizer)
    image_batch_out, label_batch_out, filename_batch = fruit_input(if_random = False, if_training = True)
    ...
    logits_out = fruit_inference(image_batch_placeholder)
#    loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=label_batch_one_hot, logits=logits_out))
    ...
    if optimizer=="GradientDescentOptimizer":
        ...
        train_step = tf.train.MomentumOptimizer(learning_rate,momentum=0.0).minimize(loss)
    else:
...
def fruit_eval():
    image_batch_out, label_batch_out, filename_batch = fruit_input(if_random = False, if_training = False)
    ...
    logits_out = tf.reshape(fruit_inference(image_batch_placeholder), [BATCH_SIZE, 5])
    ...
    with tf.Session() as sess:
        ...
        coord.join(threads)
        sess.close()
optimizers=['GradientDescentOptimizer','AdagradOptimizer','MomentumOptimizer','AdamOptimizer']
learning_rates=[0.00001,0.0001,0.001,0.01,0.10,1.00,10.00,0.00005,0.0005,0.005,0.05,0.50,5.00,50.00,0.000002,0.00002,0.0002,0.002,0.02,0.20,2.00,20.00,0.00003,0.00003,0.0003,0.003,0.03,0.30,3.00,30]
#learning_rates=[0.00001]
for i in range(len(optimizers)):
    time_list=[]
    print("Training using",optimizers[i])
    for j in range(len(learning_rates)):
        print("learning rate",learning_rates[j])
        return_time=fruit_train(optimizers[i],learning_rates[j])
        time_list.append(return_time)
        print(time_list)
    #x=[0.000001]
    x=[0.00001,0.0001,0.001,0.01,0.10,1.00,10.00,0.00005,0.0005,0.005,0.05,0.50,5.00,50.00,0.000002,0.00002,0.0002,0.002,0.02,0.20,2.00,20.00,0.00003,0.00003,0.0003,0.003,0.03,0.30,3.00,30]    
